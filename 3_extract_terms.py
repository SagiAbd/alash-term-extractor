...
Сіздің мақсатыңыз: Мәтіннен ғылыми және техникалық терминдерді, арнайы ұғымдарды, өлшем бірліктерін және кәсіби сөздерді теріп алу.
...
1. **Тақырып ауқымы**: Мәтіннің негізгі пәніне қатысты ғылыми, техникалық немесе кәсіби терминдерді алыңыз. Жалпы пәнге байланысты барлық арнайы ұғымдарды қамтыңыз.

2. **МІНДЕТТІ ТАЛАП: АНЫҚТАМАСЫ БАР ТЕРМИНДЕР ҒАНА**:
   - Терминнің мәтінде **анықтамасы** немесе **түсіндірмесі** бар ма?
   - **ИӘ**: Оны тізімге қосыңыз. (Мысалы: "Заттың бір орынды алып тұруын *сыйымсыздық* дейді").
   - **ЖОҚ**: Егер сөз жай ғана қолданылған, бірақ түсіндірілмеген болса, ОНЫ АЛМАҢЫЗ.
   - Біз автордың түсінігіне негізделген сөздік құрастырып жатырмыз.

3. **Сүзгілеу (Алуға БОЛМАЙДЫ)**:
   - Жалпы қолданыстағы сөздерді ("кітап", "бет", "білу", "адам").
...
4. **Терминге не жатады?**
   - Ғылыми шамалар мен ұғымдар.
   - Заттың немесе құбылыстың қасиеттері.
   - Арнайы кәсіби атаулар.
...
3. **field** (Сала): Ғылым саласы (көбіне "Физика").
4. **subfield** (Кіші сала): Нақты бөлімі (Механика, Оптика, Кинематика т.б.).
5. **modern_definition** (Заманауи түсініктеме): Терминнің қазіргі ғылыми анықтамасы.
6. **alash_definition** (Алаш түсініктемесі): Терминге автор берген анықтама сөйлемін мәтіннен **КӨШІРІП ЖАЗЫҢЫЗ**. Түпнұсқадан **ЕШҚАНДАЙ ӨЗГЕРІССІЗ** болуы керек. 
7. **context** (Контекст): Термин кездесетін сөйлем және оның айналасындағы 1-2 сөйлем (контекст үшін). Мәтіннен **ДӘЛМЕ-ДӘЛ КӨШІРІЛСІН (COPY-PASTE)**. Түзетуге, қысқартуға болмайды.
8. **significance** (Ғылыми маңызы): Бұл термин ғылыми тіл қалыптастыруда несімен маңызды?
...
def main():
    ...
    columns = [
        ...
        "Заманауи түсініктеме", "Алаш түсініктемесі", 
